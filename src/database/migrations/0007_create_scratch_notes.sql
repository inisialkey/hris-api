...
ALTER TABLE "scratch_notes" ADD CONSTRAINT "scratch_notes_tenant_id_tenants_id_fk" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "idx_scratch_notes_tenant_id" ON "scratch_notes" USING btree ("tenant_id");
--> statement-breakpoint
...
--
-- Dropped with the rest of the walking skeleton. The policy is here anyway,
-- because this table exists to prove the policy works.
ALTER TABLE scratch_notes ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE scratch_notes FORCE ROW LEVEL SECURITY;
--> statement-breakpoint
...
CREATE POLICY tenant_isolation ON scratch_notes
  FOR ALL
